...
function SocialIcon({ platform }) {
  const IconComponent = SOCIAL_ICON_COMPONENTS[platform] || DefaultLinkIcon
      ...
      <div className={styles.lbInner} onClick={e => e.stopPropagation()}>
        ...
        {(current.caption || current.price) && (
          <div className={styles.lbMeta}>
            <span className={styles.lbType}>{current.clothingTypeLabel || 'Piece'}{current.price ? ` - From N${current.price}` : ''}</span>
...
export function PortfolioTemplate1({ brand, photos, garmentTypes, reviews }) {
  ...
  const milestoneStats = (settings.milestones || [])
    .filter(m => m && m.label && m.number)
    .map(m => ({ value: withPlus(m.number), label: m.label }))

  const aboutStatsData = [
    ...milestoneStats,
    { value: withPlus(yearsCrafting(settings.yearFounded)), label: 'Years crafting' },
  ]

  const aboutInfoItems = [
    settings.location && { icon: 'location_on', label: 'Based in', value: settings.location },
    settings.serviceArea.length > 0 && { icon: 'local_shipping', label: 'Delivers to', value: settings.serviceArea.join(', ') },
    settings.turnaround && { icon: 'schedule', label: 'Turnaround', value: settings.turnaround },
    settings.businessHours && { icon: 'calendar_month', label: 'Hours', value: settings.businessHours },
  ].filter(Boolean)
      ...
      <section id="about" className={styles.about} ref={aboutRef}>
        <div className={styles.aboutGrid}>
          <div className={styles.aboutLeft}>
            ...
            <Reveal as="p" className={styles.aboutBio} delay={80}>
            ...
            {aboutStatsData.length > 0 && (
              <Reveal as="div" className={styles.aboutStats} delay={160}>
          ...
          {aboutInfoItems.length > 0 && (
            <Reveal as="div" className={styles.aboutRight} delay={200}>
            ...
            {filteredPhotos.map((photo, i) => (
              ...
              >
                ...
                <div className={styles.galleryCaption}>
                  ...
                  {photo.price && <span className={styles.galleryPrice}>From N{photo.price}</span>}
          ...
          <div className={styles.footerBrand}>
            ...
            {hasFooterSocials && (
              <div className={styles.footerSocials}>
                {brand.brandPhone && (
                  <a href={`https://wa.me/${brand.brandPhone.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className={styles.footerSocialBtn}>
                    <WaGlyph />
                  </a>
                )}
