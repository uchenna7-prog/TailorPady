      ...
      <div className={styles.lightboxContent} onClick={e => e.stopPropagation()}>
        <div className={styles.lightboxBar}>
          <button className={styles.lightboxBtn} onClick={onClose}>
            <span className="mi-outlined" style={{ fontSize: '1.6rem' }}>close</span>
          </button>
          <div className={styles.lightboxCounter}>{idx + 1} / {photos.length}</div>
          <button className={styles.lightboxBtn} onClick={() => onDelete(current)}>
            <span className="mi-outlined" style={{ fontSize: '1.4rem', color: 'var(--danger)' }}>delete_outline</span>
          </button>
        </div>
        ...
        <div className={styles.lightboxImgWrap}>
          {hasPrev && (
            <button className={`${styles.navBtn} ${styles.navLeft}`} onClick={() => setCurrent(photos[idx - 1])}>
              <span className="mi-outlined" style={{ fontSize: '1.6rem' }}>chevron_left</span>
            </button>
          )}
          ...
          {hasNext && (
            <button className={`${styles.navBtn} ${styles.navRight}`} onClick={() => setCurrent(photos[idx + 1])}>
              <span className="mi-outlined" style={{ fontSize: '1.6rem' }}>chevron_right</span>
            </button>
          )}
        </div>
          ...
          <div className={styles.lightboxMeta}>
            {current.price && (
              <span className={`${styles.lightboxChip} ${styles.lightboxPriceChip}`}>
                <span className="mi-outlined" style={{ fontSize: '0.75rem' }}>sell</span>
                From ₦{current.price}
              </span>
            )}
            {current.clothingTypeLabel && (
              <span className={styles.lightboxChip}>
                <span className="mi-outlined" style={{ fontSize: '0.75rem' }}>checkroom</span>
                {current.clothingTypeLabel}
              </span>
            )}
            {cat && (
              <span className={styles.lightboxChip}>
                <span className="mi-outlined" style={{ fontSize: '0.85rem' }}>{cat.icon}</span>
                {cat.label}
              </span>
            )}
            {current.customerName && (
              <span className={styles.lightboxChip}>
                <span className="mi-outlined" style={{ fontSize: '0.75rem' }}>person</span>
                {current.customerName}
              </span>
            )}
            <span className={styles.lightboxChip}>
              <span className="mi-outlined" style={{ fontSize: '0.75rem' }}>calendar_today</span>
